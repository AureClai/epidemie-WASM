...
// Drawing
func (agent *Agent) Draw(cv *Canvas) {
	gc := cv.gc
	ratio := cv.getRatio()
	x := agent.Position.X * ratio
	y := agent.Position.Y * ratio
	R := agent.Radius * ratio
	var _color color.RGBA
	switch agent.State {
	case Healthy:
		_color = color.RGBA{0x00, 0xff, 0x00, 0xff}
	case Sick:
		_color = color.RGBA{0xff, 0x00, 0x00, 0xff}
	case Recovered:
		_color = color.RGBA{0x00, 0x00, 0xff, 0xff}
	case Dead:
		_color = color.RGBA{0x00, 0x00, 0x00, 0xff}
	}

	// Params
	gc.SetFillColor(_color)
	gc.SetStrokeColor(_color)
	gc.SetLineWidth(0)
